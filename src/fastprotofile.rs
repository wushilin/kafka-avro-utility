...
impl BytesEntry {
    ...
    pub fn set_schema_id(&mut self, id: u32) {
        self.buffer[0] = 0x00;
        let bebytes = id.to_be_bytes();
        self.buffer[1..5].copy_from_slice(&bebytes);
    }
...
pub fn load_bytes_file<P: AsRef<Path>>(
    path: P,
) -> io::Result<(String, String, BytesFileIterator)> {
    ...
    // Read Descriptor Set (skip contents)
    reader.read_exact(&mut len_buf)?;
    let fds_len = u32::from_be_bytes(len_buf) as usize;
    let mut discard = vec![0u8; fds_len];
    reader.read_exact(&mut discard)?;

    // Read Message Name Length and Bytes
    reader.read_exact(&mut len_buf)?;
    ...
    Ok((schema_text, message_name, BytesFileIterator { reader }))
}
