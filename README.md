...
The toolkit consists of these main binaries:
...
4.  **`csvgen`**: Generates quoted CSV rows from a simple comma-separated column schema and field specification.
...
### 4. CSV Generator (`csvgen`)

Generates CSV rows to stdout using a simple schema file whose contents are just comma-separated column names.
...
**Key Arguments:**
*   `--schema`: Path to the CSV schema file.
*   `--field-spec`: Path to the field generation specification file. Every column in the schema must have an entry.
*   `--num-rows`: Number of data rows to generate.
*   `--add-header`: Include the header row. By default, `csvgen` emits data rows only.
*   `--threads`: Number of generator threads. Default: `5`.
*   `--batch`: Number of rows each generator thread builds before sending a batch to the printer thread. Default: `10000`.
*   `--rows-per-second`: Limit generation rate by rows per second. When omitted, no row-rate limit is applied.
*   `--mbytes-per-second`: Limit output throughput in MiB/s. When omitted, no byte-rate limit is applied.

`csvgen` uses one printer thread and a bounded channel of size `1`, so generators can stay at most one batch ahead of the printer. When both rate limits are active, `csvgen` applies both, so the more restrictive limit determines the effective speed.

When `--threads` is greater than `1`, output row order is not guaranteed because batches are printed in the order they are produced.
